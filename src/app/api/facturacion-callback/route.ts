...
// Endpoint que n8n llamará cuando termine de procesar
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { transactionId, success, data, error } = body;

    console.log('🔔 Callback recibido de n8n:', {
      transactionId,
      success,
      hasData: !!data,
      error: error || 'none'
    ...
    if (controller) {
      ...
      console.log('✅ Resultado enviado por SSE a cliente');
    } else {
      console.log('⚠️ No hay conexión SSE activa para este transactionId');
    }
