...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('🔄 API /gastos-indirectos-pirolisis iniciando...');
    ...
    // Filtro específico para Gastos (Costos Indirectos) de Pirólisis Biochar Puro en el mes anterior
    ...
    console.log('📡 Consultando Airtable con filtros específicos de Gastos Indirectos Pirólisis...');
    console.log('🔍 Mes anterior:', mesAnterior);
    console.log('🔍 Filtros aplicados:');
    console.log('   - Mes formulado =', mesAnterior);
    console.log('   - GRUPO = "Gasto" (Costos Indirectos)');
    ...
    console.log('✅ Total de registros de gastos obtenidos:', records.length);
    
    // Verificar campos en los primeros 5 registros
    if (records.length > 0) {
      console.log('');
      console.log('🔍 Muestra de los primeros 5 registros de gastos:');
    ...
    if (records.length === 0) {
      console.warn('⚠️ No se encontraron registros de gastos con el filtro aplicado para', mesAnterior);
      return NextResponse.json({ 
        records: [], 
        costosPorSemana: {},
        totales: {
        ...
        warning: `No se encontraron gastos para ${mesAnterior}`,
        success: true 
      });
    }

    // Agrupar gastos por número de semana
    ...
    console.log('📊 Resumen de procesamiento de gastos:');
    ...
    console.log(`   - Total general de gastos: $${totalGeneral.toLocaleString()}`);
    console.log(`   - Promedio por registro: $${promedioMovimiento.toLocaleString()}`);
    console.log('');
    console.log('💰 Gastos (Costos Indirectos) por semana:');
    ...
    console.error('❌ Error en API gastos-indirectos-pirolisis:', error);
    return NextResponse.json(
      { success: false, error: 'Error al obtener datos de gastos indirectos' },
      { status: 500 }
    );
  }
}