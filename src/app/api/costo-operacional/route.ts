...
export async function GET(request: NextRequest) {
  try {
    ...
    // Filtro: GRUPO = "Costo" y CLASE = "Operacional"
    const conditions = [
      `{Año formulado} = ${año}`,
      `{GRUPO} = "Costo"`,
      `{CLASE} = "Operacional"`,
    ...
    do {
      ...
      for (const record of result.records) {
        ...
        totalCostoOperacional += subtotal;
        totalRegistros++;
      }
    ...
    console.error('Error en /api/costo-operacional:', error);
    return NextResponse.json(
      { success: false, error: 'Error al obtener costo operacional' },
      { status: 500 }
    );
  }
}
